...
function render() {
  ...
  (q.gloss || []).forEach(([t, d]) => { const li = document.createElement("li"); li.innerHTML = `• <b>${t}</b> = ${d}`; ul.appendChild(li); });
  ...
  if (LIVE && CFG.LIVE_GRADING && supabase && state.user) {
    try {
      const { data, error } = await supabase.functions.invoke("grade", {
        body: { prompt: q.question, dilemma: q.dilemma, domain: q.domain, answer, confidence: conf },
      });
      if (error) {
      ...
      if (data && typeof data.validity === "number") return { v: data.validity, g: data.ingenuity, c: data.critique };
    } catch (_) { /* 폴백 */ }
  }
